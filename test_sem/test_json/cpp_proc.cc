...
int main() {
    // Create semaphores
    sem_t *sem_cpp = sem_open("/sem_cpp_json", O_CREAT, 0666, 1);  // C++ starts first
    sem_t *sem_py = sem_open("/sem_py_json", O_CREAT, 0666, 0);
    ...
    sem_close(sem_cpp);
    sem_close(sem_py);
    sem_unlink("/sem_cpp_json");
    sem_unlink("/sem_py_json");
    return 0;
}
