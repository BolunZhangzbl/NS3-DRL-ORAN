...
void BaseNetworkScenario::create_ue_nodes()
{
  ...
  NS_ASSERT(!ue_waypoints.empty());
    for (uint32_t i = 0; i < this->ue_nodes.GetN(); i++) {
        ...
        mobility->SetPosition(this->ue_waypoints[i % this->ue_waypoints.size()][i / this->ue_waypoints.size()]);
...
void BaseNetworkScenario::ue_depart_callback() {
  ...
  if (seed_number + 1 <89) {
    for (uint32_t i = 0; i < this->ue_nodes.GetN(); i++) {
        Ptr<Node> ue_node = this->ue_nodes.Get(i);
        Ptr<MobilityModel> mobility = ue_node->GetObject<MobilityModel>();
        mobility->SetPosition(this->ue_waypoints[seed_number + 1][i]);
    }
...
int main(int argc, char **argv)
{
    ...
    std::vector<std::string> seeds_tmp_2{seeds_tmp};
    for (auto i = 0; i < 1; ++i) { seeds.push_back(std::stoul(seeds_tmp_2[i])); }
